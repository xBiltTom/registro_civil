<div>
    <div>
        <a href="{{route('solicitudes.general')}}" wire:navigate><button type="button" class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 me-2 mb-2 dark:bg-blue-600 dark:hover:bg-blue-700 focus:outline-none dark:focus:ring-blue-800">Volver</button></a>
    </div>
    ...
    <div class="w-full p-6 bg-gray-100 dark:bg-gray-800 rounded-lg shadow-md">
        <form wire:submit="registrarSolicitud" class="space-y-6">
            <h2 class="text-xl font-semibold text-gray-800 dark:text-white border-b pb-2">Atendido por {{$solicitud->funcionario->persona->nombre}} {{$solicitud->funcionario->persona->apellido}}</h2>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div class="">
                    ...
                    <input readonly maxlength="8" value="{{$solicitud->pago->numero}}" name="numero_voucher" id="numero_voucher" class="w-full bg-gray-800 rounded-md border-gray-700 text-white px-3 py-2" type="text">

                </div>
                <div class="">
                    <label class="text-white block mb-1" for="fecha_voucher">Fecha del voucher</label>
                    <input readonly value="{{$solicitud->pago->fecha_voucher}}" name="fecha_voucher" id="fecha_voucher" class="w-full bg-gray-800 rounded-md border-gray-700 text-white px-3 py-2" type="text">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div class="">
                    ...
                    <input readonly maxlength="8" value="{{$solicitud->pago->pagoPeriodo->monto}}" name="monto" id="monto" class="w-full bg-gray-800 rounded-md border-gray-700 text-white px-3 py-2" type="text">

                </div>
                <div class="">
                    <label class="text-white block mb-1" for="">Voucher cargado</label>
                    <div x-data="{ showModal: false }" class="">
                        @if($solicitud->pago->ruta_voucher)
                            <button
                                type="button"
                            ...
                            >
                                <div class="bg-white dark:bg-gray-800 rounded-lg p-6 shadow-xl max-w-lg w-full relative">
                                    ...
                                    <img
                                        src="{{ asset('storage/'.$solicitud->pago->ruta_voucher) }}"
                                        alt="Voucher"
                                        class="w-full h-auto rounded-md border border-gray-300 dark:border-gray-600"
                                    >
                                    <!-- Botón de descarga -->
                                    <a
                                    href="{{ asset('storage/'.$solicitud->pago->ruta_voucher) }}"
                                    download
