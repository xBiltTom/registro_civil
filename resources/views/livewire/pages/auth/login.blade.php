...
<div class="w-[90%] max-w-[380px] bg-black/70 text-gray-200 rounded-xl p-8 shadow-xl z-10 text-center mx-auto">
    ...
    <form wire:submit="login">
        <div class="mb-4 text-left">
            ...
            <x-text-input wire:model="form.email" id="email" class="block mt-1 w-full bg-gray-800 text-gray-200 border-gray-700 focus:border-gray-500 focus:ring-gray-500" type="email" name="email" required autofocus autocomplete="username" />
        ...
        <div class="mb-4 text-left">
            ...
            <x-text-input wire:model="form.password" id="password" class="block mt-1 w-full bg-gray-800 text-gray-200 border-gray-700 focus:border-gray-500 focus:ring-gray-500" type="password" name="password" required autocomplete="current-password" />
            <x-input-error :messages="$errors->get('form.password')" class="mt-2 text-red-400" />
        </div>

        <div class="block mt-4 text-left">
            <label for="remember" class="inline-flex items-center">
                <input wire:model="form.remember" id="remember" type="checkbox" class="rounded border-gray-700 text-gray-500 shadow-sm focus:ring-gray-500">
        ...
        <div class="flex flex-col items-center justify-center mt-6 gap-4">
            ...
            @if (Route::has('password.request'))
                <a class="underline text-sm text-gray-400 hover:text-gray-300" href="{{ route('password.request') }}" wire:navigate>
    ...
    <p class="mt-6 text-sm text-gray-300">¿Eres nuevo aquí? <a wire:navigate href="{{route('register')}}" class="underline hover:text-gray-400">Regístrate</a></p>
</div>
